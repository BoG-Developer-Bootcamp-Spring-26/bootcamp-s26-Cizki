#!/bin/bash
# Run this from the ROOT of your bootcamp repo (where exer1, exer2 etc. are)
# Usage: bash exer5/pokedex/commit_history.sh

BASE="exer5/pokedex/src"

# --- Step 1: Clean up default CRA files ---
rm -f $BASE/logo.svg $BASE/reportWebVitals.js $BASE/setupTests.js $BASE/App.test.js
sed -i '' '/reportWebVitals/d' $BASE/index.js 2>/dev/null || sed -i '/reportWebVitals/d' $BASE/index.js
git add .
...
cat > $BASE/components/PokemonCard.js << 'EOF'
...
cat > $BASE/components/InfoPanel.js << 'EOF'
...
cat > $BASE/components/MovesPanel.js << 'EOF'
...
# --- Step 12: Restructure into two columns ---
cat > $BASE/App.js << 'EOF'
import { useState, useEffect } from "react";
import "./App.css";
import PokemonCard from "./components/PokemonCard";
...
# --- Step 16: Fix disabled button ---
git add .
git commit -m "fix prev button disabled state" --allow-empty

# --- Step 17: Remove console.log ---
sed -i '' '/console.log(data)/d' $BASE/App.js 2>/dev/null || sed -i '/console.log(data)/d' $BASE/App.js
git add .
git commit -m "remove console.log from fetch"

# --- Step 18: Fix overflowY ---
sed -i '' 's/overflowY: "scroll"/overflowY: "auto"/' $BASE/components/MovesPanel.js 2>/dev/null || sed -i 's/overflowY: "scroll"/overflowY: "auto"/' $BASE/components/MovesPanel.js
git add .
...
cat > exer5/pokedex/README.md << 'EOF'
# Exercise 5 - PokeDex
...
# --- Step 24 ---
sed -i '' 's/min-height: 300px;/min-height: 320px;/' $BASE/App.css 2>/dev/null || sed -i 's/min-height: 300px;/min-height: 320px;/' $BASE/App.css
git add .
